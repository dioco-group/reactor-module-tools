 ...
 * 
 * Usage: node convert-course.cjs --config courses/fsi-french/config.json
 */

const path = require('path');
const { callGemini, stripMarkdownCodeBlocks, isRateLimitError } = require('../lib/gemini-api');
const { getFiles, readTextFile, writeTextFile, fileExists, getFileSizeKB, ensureDir } = require('../lib/file-utils');
const { sleep, ProgressTracker } = require('../lib/progress');
...
function parseArgs() {
  ...
  if (configIndex === -1 || !args[configIndex + 1]) {
    console.error('Usage: node convert-course.cjs --config <path-to-config.json>');
    console.error('Example: node convert-course.cjs --config courses/fsi-french/config.json');
    process.exit(1);
  }
