...
The header (`$MODULE <title> {cover.jpg}` — title and optional cover ride the
marker line — with `TARGET_LANG_G`, `HOME_LANG_G`, optional
`DESCRIPTION`/`DIOCO_DOC_ID`/`TTS_PROMPT`; voice config; `$LESSON Title`),
...
| `{image}` on a marker title line | `$MODULE` (cover) / `$DIALOGUE` / `$SELECT` / `$PRODUCE` (activity-wide) | Block-scoped image on the marker title line (`$DIALOGUE Title {page.jpg}`): the module cover, or a shared visual shown for **ALL** lines/items (e.g. sentences about a map). An inline per-line `{page.jpg}` overrides it for that line. |
...
| `REPEAT` | `$DIALOGUE` / `$SELECT` / `$PRODUCE` (flag) | Learner repeats aloud after hearing. In `$DIALOGUE`: repeat each line. In `$SELECT`/`$PRODUCE`: after the answer is revealed, repeat the model answer (the tape's "Repeat." reinforcement) — player replays the answer audio and prompts a say-it-back; not assessed. Activity-level. |
...
| `{image}` on `$DIALOGUE` title | No | Activity-wide reference image (map/scene) on the marker title line, shown for **all** lines. Use when every line refers to one shared visual — don't attach it inline to just the first line. |
...
| `REPEAT` | No | Flag: after the answer is revealed, the learner repeats the model answer aloud (tape's "Repeat." beat). Replay + say-it-back; not assessed. |
| `{image}` on `$SELECT` title | No | Activity-wide shared reference image (diagram/map) on the marker title line. |
...
| `REPEAT` | No | Flag: after the answer is revealed, the learner repeats the model answer aloud (tape's "Repeat." beat). Replay + say-it-back; not assessed. |
| `{image}` on `$PRODUCE` title | No | Activity-wide grounding image (one visual for all items) on the marker title line. |
