...
`;
  ...
  function buildActivityRawIndex(text) {
    ...
    const map = {};
    let current = null;
    function close(endIdxInclusive) {
      if (!current)
        return;
  ...
  function renderMarkdown(md) {
    ...
    const html2 = marked.parse(withPhraseSpans, {
      gfm: true,
      breaks: false
    });
  ...
  function renderDiagnostics(diags, sourceLines) {
    ...
    const list2 = diags.length === 0 ? el("div", { class: "muted" }, ["No issues detected by linter."]) : el(
      "div",
      { class: "diagList" },
      diags.map(
        (d) => el(
    ...
    return el("details", { class: "details", open: diags.length > 0 }, [
      ...
      el("div", { class: "card innerCard" }, [header, list2])
    ]);
  }
  ...
  function renderPreviewPage(params) {
    ...
    if (!mod) {
      if (parseError)
        main.appendChild(renderError(parseError));
    ...
    const voice = el("section", { class: "card" }, [
      ...
      el("div", { class: "grid2" }, [
        ...
        formatVoiceLine("VOICE_INTRO", mod.voiceConfig.introVoice)
      ]),
      el("div", { class: "kv" }, [
    ...
    for (const [lessonIdx, lesson] of mod.lessons.entries()) {
      const lessonId = `lesson-${lessonIdx}-${slug2(lesson.title) || lesson.id}`;
      nav.appendChild(
      ...
      for (const [actIdx, activity] of lesson.activities.entries()) {
        const actId = `${lessonId}-act-${actIdx}-${slug2(activity.title) || activity.id}`;
        nav.appendChild(
  ...
  function renderActivity(activity, id, rawIndex) {
