...
export function lintModuleText(text: string): Diagnostic[] {
    ...
    const seenVoiceSpeaker: Map<string, { line: number; original: string }> = new Map(); // lower -> first seen
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (fm) {
            ...
            if (currentActivity === 'DIALOGUE') {
                ...
                if (field === 'PROMPT' && value === '') push('warning', lineNo, 'PROMPT is empty.', 'empty-prompt');
                if (field === 'RESPONSE' && value === '') push('warning', lineNo, 'RESPONSE is empty.', 'empty-response');
    ...
    if (sawModuleMarker) {
        if (!seenHeader.DIOCO_DOC_ID) push('error', 1, 'Missing required header field: DIOCO_DOC_ID', 'missing-dioco-doc-id');
        ...
        if (!seenHeader.HOME_LANG_G && !seenHeader.USER_LANG_G) push('error', 1, 'Missing required header field: HOME_LANG_G (or legacy USER_LANG_G)', 'missing-home-lang');
    }
